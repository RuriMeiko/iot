...
const char* API_ENDPOINT = "https://websocket-server-ts-production.up.railway.app/";
...
<body>
  <div class="container">
    ...
    <div class="card">
      <h2>Kết nối WiFi</h2>
      <div id="status"></div>
      <button class="btn" onclick="scanNetworks()">Quét mạng</button>
      <div id="networks"></div>
  ...
  <script>
      ...
      fetch('/scan')
        ...
        .then(data => {
          ...
          data.networks.forEach(net => {
            ...
            networksHTML += `<div class="network" onclick="selectNetwork('${net.ssid}')">
              <span>${net.ssid}</span>
        ...
        .catch(() => {
          document.getElementById('status').innerHTML = '<span class="error">Lỗi khi quét mạng</span>';
        });
    }
    ...
    function connect() {
      ...
      fetch('/connect', {
        ...
        .then(data => {
          if (data.success) {
            document.getElementById('status').innerHTML = `<span class="status">Kết nối thành công đến ${selectedSSID}!</span>`;
          } else {
            document.getElementById('status').innerHTML = `<span class="error">Kết nối thất bại</span>`;
          }
        })
        .catch(() => {
          document.getElementById('status').innerHTML = `<span class="error">Lỗi khi kết nối</span>`;
        });
    }
